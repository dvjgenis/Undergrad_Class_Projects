...
def _draw_adj_matrix(mat, mat_label, show_weights, round_digits, directed, layout='dot'):
    ...
    for j, out_node in enumerate(mat_label):
        for i, in_node in enumerate(mat_label):
            x = mat[i, j]
            if (x > 0):
                if (show_weights):
...
def gen_random_friends(n=50, p=0.97):
    A = np.random.choice(a=[0, 1], size=(n, n), p=[p, 1-p])

    A[np.random.choice(np.where(np.sum(A, axis=0) > 0)[0]), 0] = 1
